...
use crate::{aquadoggo_rpc::{connect_server::Connect, Document, NextArgs}, bus::{ServiceMessage, ServiceSender}, db::SqlStore, schema::SchemaProvider};
...
impl Connect for RpcServer {
    async fn publish(&self, request: Request<Document>) -> Result<Response<NextArgs>> {
        ...
        if self.sender.send(ServiceMessage::NewOperation(operation_id)).is_err() {
